...
function handleTreeInteraction(event) {
  ...
  const rootButton = target.closest("[data-tree-root]");
  if (rootButton) {
...
function renderExplorerHeader(branch) {
...
function renderRootSwitcher() {
  ...
  visibleRootIds.forEach((rootId) => {
    ...
    const branch = buildDescendancyBranch(rootId, new Set());
...
function renderTreeCanvas(branch) {
...
function createToggleButton(nodeId, childCount, isCollapsed) {
  const button = document.createElement("button");
  button.type = "button";
...
function createPersonCard(person, fallbackName, roleLabel) {
  ...
  if (person?.id) {
    card.href = `/person?id=${encodeURIComponent(person.id)}`;
  }
...
function buildSelectedBranch() {
  if (!selectedRootId || !peopleById.has(selectedRootId)) {
    return null;
  }
  return buildDescendancyBranch(selectedRootId, new Set());
}
function buildVisibleRoots(people) {
...
function buildDescendancyBranch(personId, visited) {
...
function collectFamilyChildIds(person, spouse) {
...
function collectCollapsibleIds(node) {
...
function countPeopleInBranch(node, seen = new Set()) {
...
function comparePeopleById(leftId, rightId) {
...
function shouldSuppressRoot(person, rootsById) {
  const spouse = getSpousePerson(person.spouse);
  if (!spouse) {
    return false;
  }
...
function formatFamilyLabel(person, spouseName) {
  if (spouseName) {
